...
<div class="block block-rounded">
  ...
  <div class="block-content block-content-full">
    ...
    <form action="{{ route('payments.store') }}" method="POST" onsubmit="return disableSubmitButton()">
      @csrf
      ...
      <div class="row justify-content-center">
        <div class="col-md-6">
          ...
          <div class="mb-4">
            ...
            <input type="text" name="number" id="number" value="{{ old('number') }}" class="form-control @error('number') is-invalid @enderror" onkeypress="return hanyaAngka(event)">
            @error('number')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <input type="text" name="holder_name" id="holder_name" class="form-control @error('holder_name') is-invalid @enderror" value="{{ old('holder_name') }}" onkeypress="return hanyaHuruf(event)">
            @error('holder_name')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
